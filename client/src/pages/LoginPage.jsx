...
const LoginPage = () => {
    ...
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl' >
...

      {
        currState === "Sign Up" && isDataSubmitted && (
          ...
          value={bio}
           rows={4} className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500' placeholder='Enter the bio..' required></textarea>
        )
      }
  ...
  {currState === "Sign Up" ? (
    <p className='text-sm text-gray-600'>
      ...
      <span onClick={()=> { setCurrState("Login"); setIsDataSubmitted(false); console.log(isDataSubmitted)}} className='font-medium text-violet-500 cursor-pointer'>
        Login here
      </span>
    </p>
  ) : (
