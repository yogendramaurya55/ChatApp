...
const ChatContainer = () => {
  ...
  const handleSendImage = async (e) =>{
    ...
    if(!file || !file.type.startsWith("image/")){
      toast.error("Select an Imgae file ")
        ...
        {messages.map((msg, index) => (
          ...
          >
                ...
                className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all  text-white 
                            ${
                              msg.senderId === authUser._id
                                ? "rounded-br-none bg-violet-400"
                                : "rounded-bl-none bg-violet-800"
                            }`}
            ...
            <div className="text-center text-xs" >
              <img src={msg.senderId !== authUser._id ? selectedUser?.profilePic || assets.avatar_icon : authUser?.profilePic || assets.avatar_icon}  alt="" className="w-7 rounded-full" />
